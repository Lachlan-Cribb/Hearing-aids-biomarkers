...
save_mean_sd <- function(df) {
  ...
  saved_mean_sd <- data.table(
    var_name = names(data[, ..non_binary]),
    mean = mean_vec,
    sd = sd_vec
  )
...
tidy_table <- function(x) {
  ...
  out <- x[
    !grep("_gcomp", parameter),
    list(outcome, parameter, estimate, lower, upper)
  ]

  out[,
    (cols) := lapply(.SD, \(.x) {
      ifelse(outcome == "S3_Abeta42_40", .x * 1000, .x)
    }),
    .SDcols = cols
  ]
...
my_summarise <- function(data, stat) {
  data |>
    ...
    group_by(Y3M_HearingAid) |>
    summarise(across(-m, mean)) |>
