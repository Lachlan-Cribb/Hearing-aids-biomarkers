...
msm_plot <- function(
  ...
  data[,
    new_effect_modifier := fcase(
      effect_modifier == "risk_score",
      "Baseline dementia risk score",
      effect_modifier == "S1_pTau181",
      "Baseline pTau181 (pg/mL)",
      effect_modifier == "S1_NFlight",
      "Baseline NfL (pg/mL)",
      effect_modifier == "S1_GFAP",
      "Baseline GFAP (pg/mL)",
      effect_modifier == "S1_Abeta42_40",
      "Baseline Abeta42:Abeta40",
      effect_modifier == "apoe_e4",
      "APOE e4 positivity",
      effect_modifier == "BLToneAvg_Better",
      "Baseline heaing loss (db HL)",
      effect_modifier == "BL_3MS_OverallScore_C",
      "Baseline 3MS overall score"
    )
  ]
    ...
    if ("Y2" %in% data$parameter) {
      data[,
        Treatment := fcase(
          ...
          parameter == "Y1",
          "Rarely/sometimes use hearing aids",
          parameter == "Y2",
          "Often/always use hearing aids"
        )
      ]
    } else {
      data[,
  ...
  if (!effect_mod == "apoe_e4") {
    plot <- ggplot(
      ...
      geom_ribbon(
        aes(ymin = lower, ymax = upper, fill = Treatment),
        alpha = 0.15,
        show.legend = FALSE
      ) +
      labs(
        x = unique(data$new_effect_modifier),
        y = ylab,
        group = "",
        colour = ""
      ) +
    ...
    plot <- ggplot(
      ...
      geom_errorbar(
        aes(ymin = lower, ymax = upper, colour = Treatment),
        position = position_dodge(0.2),
        width = 0.1,
        linetype = "dashed",
        show.legend = FALSE
