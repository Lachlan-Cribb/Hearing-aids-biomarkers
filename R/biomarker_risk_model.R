...
add_biomarker_risk <- function(
  ...
  # standardise 
  x[, (names(x[, !"Safehaven"])) := lapply(.SD, scale), 
    .SDcols = names(x[, !"Safehaven"])]
  
  # predicted hazard from fitted models
  x[,haz_dem := predict(
    dem_risk_model, newdata = x[, !"Safehaven"],
    onlySL = TRUE)$pred
  ]
  x[,haz_death := predict(
    death_risk_model, newdata = x[, !"Safehaven"],
    onlySL = TRUE)$pred
  ]
...
to_long <- function(data, final_visit, interval_duration = 2) {
  ...
  for (i in 1:final_visit) {
    data[[paste0("AV", i, "_Cens")]] <-
      if_else(is.na(data[[paste0("AV", i, "_Dem")]]) &
                (is.na(data[[paste0("AV", i, "_Death")]]) |
                   data[[paste0("AV", i, "_Death")]] == 0), 1, 0)
    if (i > 1) {
      data[[paste0("AV", i, "_Cens")]] <-
        if_else(data[[paste0("AV", i - 1, "_Cens")]] == 1, 1, 
                data[[paste0("AV", i, "_Cens")]])
    }
  }
  ...
  for (i in 1:final_visit) {
    data[[paste0("AV", i, "_Death")]] <-
      if_else(data[[paste0("AV", i, "_Dem")]] == 1 &
                !is.na(data[[paste0("AV", i, "_Dem")]]), NA, 
              data[[paste0("AV", i, "_Death")]])
  }
  
  # long format
  
  keep <-  c("Safehaven", "S1_pTau181", "S1_Abeta40", 
             "S1_Abeta42", "S1_GFAP", "S1_NFlight")
