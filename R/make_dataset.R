...
prepare_data <- function() {
  ...
  alsop[, HearingAidUse := ifelse(
    HearingAid == 0 & (HearingAidUse < 2 | is.na(HearingAidUse)), 
    1, 
    HearingAidUse)]
  
  alsop[, Y3M_HearingAidUse := ifelse(
  ...
  alsop[, CochlearImplnt := 
          ifelse(alsop$CochlearImplnt == 2, 1, alsop$CochlearImplnt)]
  ...
  hear[, BLToneAvg_Better := ifelse(BLToneAvg_Left < BLToneAvg_Right,
                                    BLToneAvg_Left,
                                    BLToneAvg_Right)]
  
  hear[, Y3MToneAvg_Better :=
         ifelse(HV3ToneAvg_Left < HV3ToneAvg_Right,
                HV3ToneAvg_Left,
                HV3ToneAvg_Right)]
  ...
  polypharm[, BL_Polypharmacy := 
              ifelse(is.na(BL_Polypharmacy), 0, BL_Polypharmacy)]
  ...
  ms_transformed <-
    lapply(c("BL", paste("AV", c(3,6,9), sep = "")), transform_ms, data = ms) |>
    as.data.table()
  
  names(ms_transformed) <-
    paste(c("BL", paste("AV", c(3,6,9), sep = "")), "3MS_OverallScore_C", sep = "_")
  ...
  death_function <- function(data, visit) {
    ...
    if (visit > 1) {
      death_out <-
        ifelse(data[[paste0("AV", visit - 1, "_Death")]] == 1, 
               1, death_out)
    }
  ...
  dem_function <- function(data, visit) {
    ifelse(data$Dementia == 1 & data$Dementia_DSR < (365 * visit),
      1,
  ...
  cancer_function <- function(data, visit) {
    ifelse(data$Cancer == 1 & data$Cancer_DSR < (365 * visit),
      1,
  ...
  frailty_vars <- frailty_vars[
    , .SD, .SDcols = patterns("Safehaven|BL|AV6|AV9")]
  ...
  bl_biomarkers[,S1_pTau181 := ifelse(
    S1_pTau181_TechErr %in% c(1,2), NA, S1_pTau181)]
  
  bl_biomarkers[,S1_Abeta40 := ifelse(
    S1_Abeta40_TechErr %in% c(1,2), NA, S1_Abeta40)]
  
  bl_biomarkers[,S1_Abeta42 := ifelse(
    S1_Abeta42_TechErr %in% c(1,2), NA, S1_Abeta42)]
  
  bl_biomarkers[,S1_GFAP := ifelse(
    S1_GFAP_TechErr %in% c(1,2), NA, S1_GFAP)]
  
  bl_biomarkers[,S1_NFlight := ifelse(
    S1_NFlight_TechErr %in% c(1,2), NA, S1_NFlight)]
  ...
  bl_biomarkers <- bl_biomarkers[, .(
    Safehaven, S1_pTau181, S1_Abeta40, 
    S1_Abeta42, S1_Abeta42_40, S1_Abeta42_lower, S1_GFAP, S1_NFlight)]
  ...
  fu_biomarkers[,S3_pTau181 := ifelse(
    S3_pTau181_TechErr %in% c(1,2), NA, S3_pTau181)]
  
  fu_biomarkers[,S3_Abeta40 := ifelse(
    S3_Abeta40_TechErr %in% c(1,2), NA, S3_Abeta40)]
  
  fu_biomarkers[,S3_Abeta42 := ifelse(
    S3_Abeta42_TechErr %in% c(1,2), NA, S3_Abeta42)]
  
  fu_biomarkers[,S3_GFAP := ifelse(
    S3_GFAP_TechErr %in% c(1,2), NA, S3_GFAP)]
  
  fu_biomarkers[,S3_NFlight := ifelse(
    S3_NFlight_TechErr %in% c(1,2), NA, S3_NFlight)]
  ...
  fu_biomarkers <- fu_biomarkers[, .(
    Safehaven, S3_pTau181, S3_Abeta40, 
    S3_Abeta42, S3_Abeta42_40, S3_Abeta42_lower, S3_GFAP, S3_NFlight)]
  ...
  df_out <- df_out |>
    select(
      -contains("DeathSub"),-contains("DeathSubType"),
      -contains("_Detail"),-contains("EventID"),-contains("Subclass"),
      -contains("MetType"),-contains("Stage"),-contains("Antihyp"),
      -starts_with("C_"), -starts_with("AV13")
    )
  ...
  df_out[, Y3M_HearingProbs := ifelse(
    Y3M_HearingProbs == 9, NA, Y3M_HearingProbs)]
   
  df_out[, Y3M_HearingProbs_1 := ifelse(
    Y3M_HearingProbs_1 == 9, NA, Y3M_HearingProbs_1)]
